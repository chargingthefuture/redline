...
Redline is an offline, gamepad-friendly platformer — a fast side-scrolling running-and-jumping
game in the spirit of the classic 16-bit speed platformers. It runs entirely in the browser and is
built to be played on a phone, a tablet, or a Chromebook without an internet connection.

### What this project is

- A single web game that runs from static files: HTML, CSS, and plain JavaScript. There is no
  server, no account, and no build step required to play.
- It installs as a home-screen app (a Progressive Web App, or PWA — a web page that a phone or
  Chromebook can save and run offline like an installed app) so it keeps working with no network.

### Stack and layout

- **Plain JavaScript, no framework and no bundler.** The game must run by opening the page directly.
  Do not add a build tool, a package manager dependency for the game itself, or a transpile step.
  Keep the code in scripts the browser loads as-is.
- **No outside code at run time.** Everything the game needs ships in this repo. Do not pull scripts,
  fonts, images, or styles from a content-delivery network or any other outside address — a
  network fetch would break offline play. All art is drawn on the canvas in code so there are no
  image files to miss.
- Rough layout: `index.html` (the page), `css/` (styles), `js/` (game code), `assets/` (the app
  icon and anything static), `manifest.webmanifest` and `sw.js` (the offline/installable app
  plumbing).

### How to run and check it

- To play locally, serve the folder over a local web address and open it — for example
  `python3 -m http.server` then open the printed address. The offline install (service worker) only
  turns on over a web address, not a bare file path.
- There is no automated test suite yet. Before you call a change done, actually run the game in a
  browser and drive the part you changed with the keyboard, then confirm the on-screen touch
  buttons and a gamepad still work. Do not rely on reading the code alone.

### Controls must stay input-agnostic

- The game must be fully playable three ways at once, on every screen: a **keyboard** (Chromebook and
  laptops), an **on-screen touch pad** (phones and tablets), and a **gamepad** (the Gamepad API).
  Any new action needs all three bindings, not just one.

### Offline is a hard requirement

- Every change must keep the game working with the network turned off after first load. If you add a
  file the game loads, add it to the service-worker cache list in `sw.js`, or offline play will
  silently break on the next visit.

### Names and likeness

- This is an original game, not a copy. Do not use the name, characters, art, music, or level layouts
  of any existing commercial game. Keep the hero and enemies original.
